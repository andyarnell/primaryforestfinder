...
| `queued` | Referenced in commented-out code or docs notes. NOT loaded today. Listed for workshop discussion + future swap-in. Search `Queued:` to find them. |
...
### `Queued:` WSF 2015 / WSF 2019 (DLR) — `queued`
Single-year settlement masks. Not used; commented-out alternatives at `modules/timeSeriesAnthro.js:332-333`.
...
### `Queued:` USA TIGER Roads / WUR Congo Logging Roads / Microsoft Global Roads / Ghost Roads Asia / MapBiomas Brazil Roads — all `queued`
Helpers / commented-out alternatives. See JSON for assets + citations.
...
## `Queued:` ancillary — workshop / swap-in

| Dataset | Status | Asset | Notes |
|---|---|---|---|
| WRI SBTN Natural Lands v1.1 (2020) | `queued` | `WRI/SBTN/naturalLands/v1_1/2020` | Mazur et al. 2025; CC-BY-SA 4.0 |
| European Primary Forests Database v2 | `queued` | `HU_BERLIN/EPFD/V2/polygons` | Sabatini et al. 2018 |
| Tsinghua DESS China Terrace Map v1 | `queued` | `Tsinghua/DESS/ChinaTerraceMap/v1` | Cao et al. 2021 |
| FDAP Palm/Rubber/Cocoa | `optional_default_off` | `projects/forestdatapartnership/assets/{palm,rubber,cocoa}/model_2024a` | See plantations section |

Search `"status": "queued"` in [`datasets_global.json`](datasets_global.json) for the full list.
...
4. Update this Markdown's status badges to match.

When a citation needs updating (new DOI, new release year), update `citation` in the JSON; the Markdown can be regenerated from JSON if a script is added.
