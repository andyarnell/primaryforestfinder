...
## v4.16.0-beta.16 (methods note in Save to computer + single-year handling)
- **Run records moved to "Save to computer".** The **methods note** (`.md`) and **run metadata** (`.json`) downloads now live under a compact "This run" row in the Save-to-computer section (where the layer downloads are), not Config. Reason: these in-browser downloads work in the **published app**, whereas the Outputs Drive-export tickboxes do nothing there (no Tasks tab). Config is now just Save / Load Settings + Reset. A dedicated **"Methods note"** export tickbox is also present in Outputs (Drive) — the follow-up promised in beta.15.
- **Single-year scope for Save to computer.** That section operates one year at a time: a notice states the year in use ("Single year only — uses Year `<Y>`") and, in Compare-Years mode, turns amber and points to Save to Drive (which writes per-year files). The methods note now prints an explicit **"Year analysed"** line and, when Compare was on, a caveat that it covers one year only. Fixed a bug where the record downloads read the hidden `yearSelector1` (default 2000) instead of the visible single-mode `yearSelector2`.
- **Download warnings** (mirroring Area Statistics): no-country / analysis-not-run notices on the methods-note, run-metadata and Save-Settings downloads (Load Settings excluded). Stale download notes/links now clear when the layer or resolution changes, and the "download workaround" Note + Drive Tip stay hidden when no download happens.
- **Methods note content:** author-year (Harvard) in-text citations + an alphabetical reference list; ASCII-only (fixes CSV/Excel mojibake); an **FRA 2025 definitions** block with source link; a **static-input** marker (`*`) flagging datasets applied the same regardless of the analysis year (SDPT, OSM, ALOS DEM, GAUL); plain wording ("buffered to represent likely human influence") and "Refine Output (ecological-viability)" terminology. Dropped the jargon "(provenance)" from user-facing labels.
- **Refine Input** category dropdown now defaults to **Tree cover** (and Reset Defaults matches).
- Added `CHL` to the source-simplify iso3 exceptions in `getCountryFeatures()`.

## v4.16.0-beta.15 (provenance methods narrative)
- New `modules/provenanceNarrative.js`: generates a human-readable Markdown "methods note" for a run — a prose paragraph describing the datasets + processing chain (tree cover → Forest → naturally regenerating → primary), with inline citations, an FRA-alignment caveats block (incl. the SDPT-v2 rubber/class-2 limitation), and a numbered reference list built from only the datasets actually used. Adapts to the run (source, thresholds, buffers, slope/WDPA, smoothing, scale) and supports short/extended lengths.
- Wired in: the narrative is embedded in the run-metadata bundle (`run__provenance_md`) so it ships in the metadata JSON, is printed to the Console (on "Download Run Metadata" and on Drive export), and a new **"📄 Download methods note"** button downloads it as a clean `.md` text file via the data-URI `targetUrl` trick (real newlines, no JSON escaping). Also lists each exported layer with its datasets + recipe ("Layers in this export"). (v1: a dedicated export tickbox + per-toggle step-dropping are follow-ups.)
- Citations mirror `docs/datasets_global.json` — keep both in sync.

## v4.16.0-beta.14 (rename 02a tree-cover exports)
- Renamed `02a_forest_raw` → `02a_tree_cover_binary` (the thresholded 1/0 tree-cover input). It was being confused with the genuine Hansen/GLAD raw source exports (`02a_hansen_treecover2000_raw`, `02a_hansen_lossyear_raw`, `02a_glad_tree_height_m`) because it shared the `02a` prefix and the `raw` suffix. New convention: `raw` = continuous source, `binary` = thresholded mask. Export checkbox "Tree cover (raw input)" → "Tree cover (binary)". The binary export now carries a band name recording the source + the threshold(s) that source uses (e.g. `tree_cover_glad_h5m`, `tree_cover_hansen_c10pct`).
- Renamed `02a_glad_tree_height_m` → `02a_glad_tree_height_raw` to bring the continuous sources under the `raw` convention (the `_m` unit is preserved in the band name).
- The QGIS plugin + batch tools accept BOTH old `forest_raw` and new `tree_cover_binary`, so already-exported files still auto-match.
