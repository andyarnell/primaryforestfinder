...
## v4.16.0-beta.17 (methods note out of run-metadata JSON)
- **Run-metadata JSON is now a clean config snapshot.** The methods-note / provenance narrative (`run__provenance_md`) is no longer embedded in `gee_run_metadata_*.json`. The narrative re-flows on minor wording tweaks and changed far more often than the config keys, which bloated the JSON and made it hard to read. It is now generated by a dedicated `buildProvenanceMd()` and shipped only as its own `.md` methods-note sidecar (Save to Drive) and the in-browser **Methods note** download — no user-facing output is lost.
